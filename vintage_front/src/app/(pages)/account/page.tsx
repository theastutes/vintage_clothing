...
const page = async () => {
  ...
  if (!session?.user) {
    return (
      <div className="flex flex-col bg-brown-secondary justify-center items-center text-mywhite h-screen w-full">
        <p className="text-4xl mb-8">Seems Like You haven&apos;t Signed Up Yet</p>
  ...
  return (
    ...
    <div className="h-screen w-screen sm:pl-16 max-sm:pt-14 flex sm:flex-row flex-col overflow-hidden justify-center items-center  gap-4">
      ...
      <div className="h-screen select-none relative overflow-hidden overflow-y-scroll w-full flex flex-col gap-2 sm:flex-row items-center text-2x p-2">
        ...
        <div className="sm:w-1/3 sm:h-full h-[25%] flex items-center justify-center w-full p-2">
          <div className=" flex flex-col gap-2 w-full h-full  sm:h-[80%] p-2 shadow-lg shadow-gray-300 rounded-3xl">
            ...
            <div className="flex flex-row items-center justify-between"><div>{session?.user?.name!}</div><form
              action={async () => {
                "use server"
          ...
          <div className="grid grid-flow-col grid-cols-2 grid-rows-2 gap-2 sm:gap-4 p-2  sm:h-[80%] h-[59%] w-full text-xs">
            <div className="relative shadow-md shadow-gray-500 bg-black/80 rounded-2xl  overflow-hidden">
              ...
              <Suspense fallback={<Loading />}>
                <Image
                  src={"https://images.unsplash.com/photo-1586449480558-33ae22ffc60d?q=80&w=1770&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D"}
                  alt=""
                  height={200}
                  width={200}
            ...
            <Link href={'/addresses'} className="relative shadow-md shadow-gray-500 bg-black/80 rounded-2xl  overflow-hidden">
              <div className="opacity-0 hover:opacity-100 rounded-2xl transition-all duration-700 absolute flex items-center justify-center text-2xl font-extralight bg-white/5 backdrop-blur-sm left-0 right-0 top-0 bottom-0">
                Address
              </div>
              <Suspense fallback={<Loading />}>
            ...
            <div className="relative shadow-md shadow-gray-500 bg-black/80 rounded-2xl  overflow-hidden">
              ...
              <Suspense fallback={<Loading />}>
                <Image
                  src={"https://plus.unsplash.com/premium_vector-1682298485852-79eb8ffe4320?bg=FFFFFF&w=900&auto=format&fit=crop&q=60&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8MjF8fG9yZGVyc3xlbnwwfHwwfHx8MA%3D%3D"}
                  alt=""
                  height={200}
                  width={200}
                  className="h-full w-full"
